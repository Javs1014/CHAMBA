// src/components/proforma/proforma-document.tsx
'use client';
import type { Proforma, Client } from '@/types';
import { format, parseISO } from 'date-fns';
import { SuccessfulTradeLogo } from './successful-trade-logo';
import { companyDetails } from '@/config/company-details';
import { useState, useEffect } from 'react';

interface ProformaDocumentProps {
  proforma: Proforma;
  client?: Client;
}

// Helper to format currency
const formatCurrency = (amount: number | undefined, decimals = 2) => {
  if (amount === undefined) return '0.00';
  return amount.toLocaleString('en-US', { minimumFractionDigits: decimals, maximumFractionDigits: decimals });
};

const formatDate = (dateString: string | undefined) => {
  if (!dateString) return 'N/A';
  try {
    return format(parseISO(dateString), 'MMMM d, yyyy'); // e.g., May 26, 2025
  } catch (error) {
    console.error("Error formatting date:", dateString, error);
    return dateString;
  }
};


// --- Main Component ---
export function ProformaDocument({ proforma, client }: ProformaDocumentProps) {
  if (proforma.company === 'Successful Trade') {
    return <SuccessfulTradeProforma proforma={proforma} client={client} company={companyDetails['Successful Trade']} />;
  }
  
  // Default to Trade Evolution template
  return <TradeEvolutionProforma proforma={proforma} client={client} company={companyDetails['Trade Evolution']} />;
}


// --- Successful Trade Proforma Template ---
function SuccessfulTradeProforma({ proforma, client, company }: { proforma: Proforma, client: Client | undefined, company: typeof companyDetails['Successful Trade']}) {
  return (
    <div className="bg-card p-8 shadow-lg rounded-md w-full max-w-4xl mx-auto my-8 font-sans text-sm">
      <SuccessfulTradeLogo className="mb-6 h-auto" />

        {/* Title and PI Info */}
        <div className="flex justify-end mb-6">
          <div className="w-1/2">
            <h2 className="text-xl font-bold text-right mb-2">PROFORMA INVOICE</h2>
            <div className="grid grid-cols-2 gap-x-4 text-sm">
                <span className="font-bold">PI #</span><span className="">{proforma.proformaNumber}</span>
                <span className="font-bold">DATE</span><span className="">{formatDate(proforma.issuedDate)}</span>
                <span className="font-bold">CURRENCY</span><span className="">{proforma.currency}</span>
                <span className="font-bold">CUST. REF #</span><span className="">{proforma.reference || 'N/A'}</span>
            </div>
          </div>
        </div>

        {/* Bill To / Ship To */}
        <div className="grid grid-cols-2 gap-8 mb-4">
            <div>
                <p className="font-bold mb-1">BILL TO:</p>
                <p className="font-semibold">{proforma.clientName}</p>
                <p className="whitespace-pre-wrap">{proforma.clientAddress}</p>
                {proforma.clientTaxId && <p>Tax ID: {proforma.clientTaxId}</p>}
            </div>
            <div>
                <p className="font-bold mb-1">SHIP TO:</p>
                <p className="font-semibold">{proforma.shipToName || proforma.clientName}</p>
                <p className="whitespace-pre-wrap">{proforma.shipToAddress || proforma.clientAddress}</p>
                {proforma.shipToTaxId && <p>Tax ID: {proforma.shipToTaxId}</p>}
            </div>
        </div>

        {/* Items Table */}
        <div>
            <table className="w-full border-collapse">
                <thead>
                    <tr className="bg-[#DCF2D9]">
                        <th className="p-2 text-left font-bold w-[10%]">QTY.</th>
                        <th className="p-2 text-left font-bold w-[10%]">UNIT</th>
                        <th className="p-2 text-left font-bold w-[55%]">DESCRIPTION</th>
                        <th className="p-2 text-right font-bold w-[12.5%]">UNIT PRICE</th>
                        <th className="p-2 text-right font-bold w-[12.5%]">AMOUNT</th>
                    </tr>
                </thead>
                <tbody>
                    {proforma.items.map((item, index) => (
                        <tr key={index} className="border-b">
                            <td className="p-2 align-top">{item.quantity}</td>
                            <td className="p-2 align-top">{item.unit}</td>
                            <td className="p-2 align-top whitespace-pre-wrap">
                                <b>{item.productName}</b>
                                {item.description && <p>{item.description}</p>}
                            </td>
                            <td className="p-2 text-right align-top">{formatCurrency(item.unitPrice, 3)}</td>
                            <td className="p-2 text-right align-top">{formatCurrency(item.totalPrice)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
        </div>

        {/* Totals Section */}
        <div className="flex justify-end mt-2">
            <div className="w-1/3">
                <div className="flex justify-between">
                    <span>SUBTOTAL</span>
                    <span>{formatCurrency(proforma.subTotal)}</span>
                </div>
                <div className="flex justify-between">
                    <span>TAX</span>
                    <span>{formatCurrency(proforma.taxAmount)}</span>
                </div>
                <div className="flex justify-between font-bold border-t mt-1 pt-1">
                    <span>TOTAL</span>
                    <span>{formatCurrency(proforma.grandTotal)}</span>
                </div>
            </div>
        </div>
        
        {/* Signature Section */}
        <div className="flex justify-between mt-16">
            <div className="w-1/2">
                <p className="font-bold">CUSTOMER SIGNATURE:</p>
                <div className="border-b border-black mt-12 h-px"></div>
                <p>{proforma.customerSignatoryName || proforma.clientName}</p>
            </div>
            <div className="w-1/3 text-right">
                <p className="font-bold">AUTHORIZED BY:</p>
                <div className="mt-8">
                  <p>{company.signature.name}</p>
                  <p className="text-xs">{company.signature.title}</p>
                </div>
            </div>
        </div>

        {/* Bank & Terms */}
        <div className="grid grid-cols-2 gap-8 mt-12 pt-4 border-t border-black">
            <div>
                <p className="font-bold mb-2">BANK DETAILS</p>
                <p>Account holder: {company.bankDetails.accountHolder}</p>
                <p>Account number: {company.bankDetails.accountNumber}</p>
                <p>SWIFT: {company.bankDetails.swift}</p>
                <p>Bank name: {company.bankDetails.bankName}</p>
                <p>Bank address: {company.bankDetails.bankAddress}</p>
            </div>
            <div>
                <p className="font-bold mb-2">TERMS & CONDITIONS</p>
                <div className="grid grid-cols-[max-content_1fr] gap-x-2 gap-y-0.5">
                    <span>Port of Loading:</span><span className="font-medium">{proforma.portAtOrigin || 'N/A'}</span>
                    <span>Port of Arrival:</span><span className="font-medium">{proforma.portOfArrival || 'N/A'}</span>
                    <span>Final Destination:</span><span className="font-medium">{proforma.finalDestination || 'N/A'}</span>
                    <span>Containers:</span><span className="font-medium">{proforma.containers || 'N/A'}</span>
                    <span>Payment Terms:</span><span className="font-medium">{proforma.paymentTerms || 'N/A'}</span>
                    <span>Delivery:</span><span className="font-medium">{proforma.delivery || 'N/A'}</span>
                </div>
                {proforma.notes && <p className="mt-2 text-xs italic">{proforma.notes}</p>}
            </div>
        </div>

        {/* Global note */}
        <div className="text-center mt-4">
            <p className="text-xs">{company.footerText}</p>
        </div>

        {/* Footer */}
        <div className="text-center font-semibold mt-8 pt-2 border-t border-black">
            <p>{company.website}</p>
        </div>
         <style jsx global>{`
          @media print {
            body { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          }
        `}</style>
    </div>
  );
}


// --- Trade Evolution Proforma Template ---
function TradeEvolutionProforma({ proforma, client, company }: { proforma: Proforma, client: Client | undefined, company: typeof companyDetails['Trade Evolution']}) {
  const currencySymbol = proforma.currency.startsWith('USD') ? '$' : '';
  const logoBase64 = 'data:image/png;base64,';

  return (
    <div className="bg-card p-10 shadow-lg rounded-md w-full max-w-4xl mx-auto my-8 font-sans text-xs print:shadow-none" style={{ fontFamily: 'Calibri, sans-serif' }}>
        <style jsx global>{`
            .calibri-font { font-family: 'Calibri', sans-serif; }
            .blue-line { border-top: 2px solid #0B5E8E; }
            .label { font-weight: bold; text-transform: uppercase; }
        `}</style>
        
        {/* Header */}
        <div className="flex justify-between items-start mb-4">
            <div className="calibri-font">
                <img src={logoBase64} alt="Trade Evolution Logo" style={{ width: '150px', height: 'auto' }} />
            </div>
            <div className="text-right text-[10px] text-[#0B5E8E] calibri-font">
                <p>TradeEvolution OÜ &nbsp;&nbsp; TAX ID {company.taxId} &nbsp;&nbsp; Phone {company.phone}</p>
                <p>ADDRESS: {company.address}, Estonia</p>
            </div>
        </div>

        <div className="blue-line mb-2"></div>
        
        {/* Proforma Info */}
        <div className="flex justify-between items-start calibri-font">
            <div>
                <p><span className="label">Issued at:</span> Tallinn, Estonia, {formatDate(proforma.issuedDate)}</p>
            </div>
            <div className="text-right">
                <p className="label text-[#0B5E8E] text-sm">Sales Order</p>
                <p className="text-base">{proforma.proformaNumber}</p>
            </div>
        </div>
        
        <div className="blue-line my-2"></div>
        
        {/* Client Info */}
        <div className="grid grid-cols-2 gap-4 calibri-font">
            <div>
                <p><span className="label">Sold to:</span> {proforma.clientName}</p>
                <p><span className="label">Address:</span> <span className="whitespace-pre-wrap">{proforma.clientAddress}</span></p>
                <p><span className="label">Tax ID:</span> {proforma.clientTaxId}</p>
            </div>
             <div>
                <p><span className="label">Ship to:</span> {proforma.shipToName || proforma.clientName}</p>
                <p><span className="label">Address:</span> <span className="whitespace-pre-wrap">{proforma.shipToAddress || proforma.clientAddress}</span></p>
                <p><span className="label">Tax ID:</span> {proforma.shipToTaxId || proforma.clientTaxId}</p>
            </div>
        </div>

        <div className="text-right calibri-font mt-1"><strong>Currency:</strong> {proforma.currency}</div>
        <div className="blue-line my-2"></div>
        
        {/* Items Table */}
        <table className="w-full border-collapse calibri-font">
            <thead>
                <tr>
                    <th className="text-left label text-[#0B5E8E] w-[8%] pb-1">QTY.</th>
                    <th className="text-left label text-[#0B5E8E] w-[12%] pb-1">UNIT</th>
                    <th className="text-left label text-[#0B5E8E] w-[48%] pb-1">DESCRIPTION</th>
                    <th className="text-left label text-[#0B5E8E] w-[16%] pb-1">UNIT PRICE</th>
                    <th className="text-left label text-[#0B5E8E] w-[16%] pb-1">EXT. PRICE</th>
                </tr>
            </thead>
            <tbody>
                {proforma.items.map((item, index) => (
                    <tr key={index} className="border-b border-gray-200">
                        <td className="py-1 align-top">{item.quantity}</td>
                        <td className="py-1 align-top">{item.unit}</td>
                        <td className="py-1 align-top">
                            <p className="font-bold">{item.productName}</p>
                            <p className="text-[10px] whitespace-pre-wrap">{item.description}</p>
                        </td>
                        <td className="py-1 align-top">{currencySymbol}{formatCurrency(item.unitPrice, 3)}</td>
                        <td className="py-1 align-top">{currencySymbol}{formatCurrency(item.totalPrice)}</td>
                    </tr>
                ))}
            </tbody>
        </table>
        
        {/* Observation */}
        {proforma.notes && (
            <div className="mt-2 calibri-font">
                <strong>Observation:</strong>
                <p className="whitespace-pre-wrap">{proforma.notes}</p>
            </div>
        )}
        
        <div className="blue-line my-2"></div>
        
        {/* Sales Detail */}
        <div className="grid grid-cols-2 gap-8 mt-2 calibri-font">
            <div>
                <p className="label">SALES DETAIL</p>
                <p>Port at Origin: {proforma.portAtOrigin}</p>
                <p>Port of Arrival: {proforma.portOfArrival}</p>
                <p>Final Destination: {proforma.finalDestination}</p>
                <p>Reference: {proforma.reference}</p>
                <p className="mt-2">Payment Terms: {proforma.paymentTerms}</p>
                <p>Delivery: {proforma.delivery}</p>
            </div>
            <div>
                <p>Vessel: {proforma.vessel}</p>
                <p>Containers: {proforma.containers}</p>
                <p>Container No.: {proforma.containerNo}</p>
            </div>
        </div>

        <p className="text-center font-bold mt-2 calibri-font">*Travel and delivery times are always estimated</p>
        
        {/* Totals */}
        <div className="flex justify-end mt-2 calibri-font">
            <div className="w-48 text-right font-bold">
                <div className="flex justify-between"><span>SUBTOTAL</span><span>{currencySymbol}{formatCurrency(proforma.subTotal)}</span></div>
                <div className="flex justify-between"><span>TAX</span><span>{currencySymbol}{formatCurrency(proforma.taxAmount)}</span></div>
                <div className="flex justify-between"><span>TOTAL</span><span>{currencySymbol}{formatCurrency(proforma.grandTotal)}</span></div>
            </div>
        </div>
        
        {/* Bank Details */}
        <div className="mt-4 calibri-font text-[10.5px]">
            <p><strong>USD Account Details:</strong></p>
            <p>Account holder: {company.usdAccountDetails.accountHolder}</p>
            <p>Wire routing number: {company.usdAccountDetails.wireRoutingNumber}</p>
            <p>Account number: {company.usdAccountDetails.accountNumber}</p>
            <p>Account type: {company.usdAccountDetails.accountType}</p>
            <p>SWIFT Code: {company.usdAccountDetails.swiftCode}</p>
            <p className="whitespace-pre-wrap">Bank address: {company.usdAccountDetails.bankAddress.join('\n')}</p>
        </div>
        
        {/* Signatures */}
        <div className="grid grid-cols-2 gap-8 mt-10 calibri-font">
            <div>
                <p className="font-bold">CUSTOMER:</p>
                <div className="mt-8 border-t border-[#0B5E8E]"></div>
                <p>{proforma.customerSignatoryName || proforma.clientName}</p>
            </div>
            <div className="text-left">
                <p className="font-bold">AUTHORIZATION:</p>
                <p className="mt-4">{company.signature.name}</p>
                <p>{company.signature.title}</p>
            </div>
        </div>
    </div>
  );
}
