...
import { useProformas, useDeleteProforma } from '@/hooks/use-proformas'; // ← agregado useDeleteProforma
...
export default function ProformasPage() {
  ...
  // Función para eliminar proforma
  const handleDeleteProforma = async (proformaId: string) => {
    if (!window.confirm("¿Estás seguro de que quieres eliminar esta proforma? Esta acción no se puede deshacer.")) {
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: "No se pudo eliminar la proforma. Intenta nuevamente.",
        variant: "destructive",
      });
    }
  };
                ...
                {filteredProformas.map((proforma) => (
                  <TableRow key={proforma.id}>
                    ...
                    <TableCell className="text-right space-x-2">
                      ...
                      >
                        {deleteProformaMutation.isPending ? 'Eliminando...' : 'Delete'}
                      </Button>
                    </TableCell>
                  </TableRow>
                ))}
