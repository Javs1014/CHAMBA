...
export default function EditClientPage() {
  ...
  const onSubmit = async (data: ClientFormValues) => {
    if (!client) return;
    try {
      await updateClientMutation.mutateAsync({ ...client, ...data });
      toast({
        title: "Client Updated",
        description: `Client "${data.companyName}" has been successfully updated.`,
      });
      router.push('/clients');
    } catch (error) {
      toast({
  ...
  if (isLoadingClient) {
      return (
          <div>
            <PageHeader title="Loading Client..." actions={<Skeleton className="h-10 w-24" />} />
            <Card>
  ...
  if (!client) {
    return (
      <div>
        <PageHeader title="Client Not Found" description="The client you are trying to edit does not exist." />
    ...
    <div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex justify-end">
          <Button type="submit" size="lg" disabled={updateClientMutation.isPending}>
